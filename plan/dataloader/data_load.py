...
Batch = namedtuple('Batch', 'Observations Actions LLM Knowledge')
...
class PlanningDataset(Dataset):
    ...
    def sample_single(self, index):
        ...
        graph_path = torch.tensor(graph_path, dtype=torch.long)

        LLM_path = torch.tensor(LLM_path, dtype=torch.long)
        return frames, labels_tensor, LLM_path, graph_path

    def __getitem__(self, index):
        frames, labels, LLM_path, graph_path = self.sample_single(index)
        batch = Batch(frames, labels, LLM_path, graph_path)
        return batch
