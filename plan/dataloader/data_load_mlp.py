...
class PlanningDataset(Dataset):
    def __init__(self,
        ...
        self.is_val = is_val
        #self.data_root = '/l/users/ravindu.nagasinghe/datasets/PDPP/CrossTask_assets'
        self.data_root = root
        self.args = args
        ...
        if args.dataset == 'crosstask':
            ...
            if os.path.exists(cross_task_data_name):
                ...
                for idx in range(len(all_vids)):
                    ...
                    #################################edited#############################
                    action_list = [sublist[-1] for sublist in legal_range]
                    data_to_print = {
                            
                            'vid': vid,
                            'task': task,
                            'legal_range': legal_range,
                            'task_id': self.task_class[task],
                    ...
                    action_list_print = {
                            ...
                            'vid': vid,
                            'task': task,
                            'legal_range': action_list,
                    }
                    # Open the file in append mode and write the JSON data on a new line
                    with open('output_file.txt', 'a') as file1:
                        json.dump(data_to_print, file1)
                        file1.write('\n')  # Add a newline character to separate entries

                    with open('testing_action_list.txt', 'a') as file2:
                        json.dump(action_list_print, file2)
                        file2.write('\n')  # Add a newline character to separate entries
    ...
    def curate_dataset(self, images, legal_range, M=2):
        ...
        if end_idx >= len(images)-1:
            ...
            image_end = images[end_idx - 1:end_idx + M - 1]  
