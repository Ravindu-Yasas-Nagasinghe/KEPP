    ...
    for i_batch, sample_batch in enumerate(val_loader):
        ...
        with torch.no_grad():
            ...
            for i in range(len(video_label)):
                if (torch.equal(video_label[i], argmax_index[i])):
                    bs = 256
                    index_vid_sim = (index_differing*bs)+i
                    similar_sublists[index_vid_sim] = {
                        'i_batch': i_batch,
            ...
            for i in range(len(video_label)):
                if not (torch.equal(video_label[i], argmax_index[i])):
                    bs = 256
                    index_vid = (index_differing*bs)+i
                    differing_sublists[index_vid] = {
                        'i_batch': i_batch,
                        'video_label_list': video_label[i].tolist(),
                        'predicted_list': argmax_index[i].tolist(),
                        'predicted_sequence': output_list_action_sequence[i],
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    temporal_model = temporal.TemporalUnet(
        args.action_dim + args.observation_dim + args.class_dim_llama + args.class_dim_graph,
        dim=256,
        dim_mults=(1, 2, 4), )
