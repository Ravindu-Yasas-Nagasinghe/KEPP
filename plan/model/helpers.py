...
class Downsample1d(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.conv = nn.Conv1d(dim, dim, 2, 1, 0) #####Edited kernal size previously had (dim, dim, 2, 1, 0)
...
class Upsample1d(nn.Module):
    def __init__(self, dim):
        super().__init__()
        print(dim, 'down!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
        self.conv = nn.ConvTranspose1d(dim, dim, 2, 1, 0) #####Edited kernal size previously had (dim, dim, 2, 1, 0)
...
def condition_projection(x, conditions, action_dim, LLM_dim , PKG_dim):
    for t, val in conditions.items():
        #print('t---------------',t)
        if t != 'PKG_action_path' and t != 'LLM_action_path':
            x[:, t, LLM_dim + PKG_dim + action_dim:] = val.clone()

    x[:, 1:-1, LLM_dim + PKG_dim + action_dim:] = 0.
    x[:, :, :PKG_dim] = conditions['PKG_action_path']
    x[:, :, PKG_dim:PKG_dim + LLM_dim] = conditions['LLM_action_path']
...
class Weighted_MSE(nn.Module):
    ...
    def __init__(self, weights, action_dim, LLM_dim, PKG_dim):
        super().__init__()
    ...
    def forward(self, pred, targ):
        ...
        loss_action[:, 0, self.LLM_dim + self.PKG_dim: self.LLM_dim + self.PKG_dim + self.action_dim] *= 5.
        loss_action[:, -1,  self.LLM_dim + self.PKG_dim: self.LLM_dim + self.PKG_dim + self.action_dim] *= 5.

        #loss_action[:, 1:-1, self.class_dim:self.class_dim + self.action_dim] *= 0.
