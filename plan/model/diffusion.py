...
class GaussianDiffusion(nn.Module):
    def __init__(self, model, horizon, observation_dim, action_dim, LLM_dim, PKG_dim, n_timesteps=200,
        ...
        self.loss_fn = Losses[loss_type](None, self.action_dim, self.LLM_dim, self.PKG_dim)
    ...
    def p_sample_loop(self, cond, if_jump):
        ...
        shape = (batch_size, horizon, self.PKG_dim + self.LLM_dim + self.action_dim + self.observation_dim)
        ...
        x = condition_projection(x, cond, self.action_dim, self.LLM_dim, self.PKG_dim)
    ...
    def p_losses(self, x_start, cond, t):
        ...
        x_noisy = condition_projection(x_noisy, cond, self.action_dim, self.LLM_dim, self.PKG_dim)

        x_recon = self.model(x_noisy, t)
        x_recon = condition_projection(x_recon, cond, self.action_dim, self.LLM_dim, self.PKG_dim)
