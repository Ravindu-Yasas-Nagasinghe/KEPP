...
def validate_graph_final(testing_sequences, graph_):
    ...
    paths = find_top_n_paths(graph_, start, end,n=2, max_path_length=6)
        #if paths:
    path_Set.append([item[0] for item in paths])
    if (len(path_Set[0])==0):
...
if __name__ == "__main__":
    ...
    if mode == 'train_minmax':
        ...
        videos=[]
        vids =[]
        with open('/l/users/ravindu.nagasinghe/Architecture/T6/stepmodel/final_test_data_list.json', 'r') as file:
            data = json.load(file)
            with open(graph_save_path, 'rb') as graph_file:
