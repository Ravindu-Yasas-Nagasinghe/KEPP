    ...
    def train(self, n_train_steps, if_calculate_acc, args, scheduler):
        ...
        for step in range(n_train_steps):
            for i in range(self.gradient_accumulate_every):
                ...
                #print('shape: ', batch[1].shape , batch[0].shape)
                global_img_tensors = batch[0].cuda().contiguous().float()
                img_tensors = torch.zeros((bs, T, args.action_dim + args.observation_dim))
                img_tensors[:, 0, args.action_dim:] = global_img_tensors[:, 0, :]
                img_tensors[:, -1, args.action_dim:] = global_img_tensors[:, -1, :]
                ...
                img_tensors[:, :, :args.action_dim] = action_label_onehot
        ...
        if if_calculate_acc:
            with torch.no_grad():
                ...
                actions_pred = output[:, :, :self.model.module.action_dim]\
