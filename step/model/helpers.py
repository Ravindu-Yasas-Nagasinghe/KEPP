...
class Downsample1d(nn.Module):
    def __init__(self, dim):
        super().__init__()
        print(dim, 'down!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
        self.conv = nn.Conv1d(dim, dim, 2, 1, 0) #####Edited kernal size previously had (dim, dim, 2, 1, 0) . make 2 to 1 when horizon is 2.
...
class Upsample1d(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.conv = nn.ConvTranspose1d(dim, dim, 2, 1, 0) #####Edited kernal size previously had (dim, dim, 2, 1, 0) .  make 2 to 1 when horizon is 2.
...
def condition_projection(x, conditions, action_dim):
    for t, val in conditions.items():
        if t != 'task':
            x[:, t, action_dim:] = val.clone()

    x[:, 1:-1, :] = 0.      ########################condition padding
...
class Weighted_MSE(nn.Module):
    ...
    def forward(self, pred, targ):
        ...
        loss_action[:, 0, :self.action_dim] *= 10.
        loss_action[:, -1, :self.action_dim] *= 10.
        loss_action[:, 1:-1, :self.action_dim] *= 0.
